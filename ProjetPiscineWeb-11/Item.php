...
<?php 
	...
	if($db_found)
	{
	  ...
	  $resultVendeur = mysqli_query($db_handle, "SELECT * FROM vendeur WHERE `email_vendeur` = $id" ); 
	}
	else 
	{
	...
	while ($rowVendeur = mysqli_fetch_array($resultVendeur, MYSQLI_ASSOC))
	{
	    $tableauVendeur[] = $rowVendeur;
	} 
...
<head>
	...
	$checkVendeur =false; 
	for($i=0;$i< count($tableauVendeur) ;$i++)
	{
		if ( $_SESSION['email'] == $tableauVendeur[$i]['email_vendeur'])
		{
